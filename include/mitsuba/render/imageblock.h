...
public:
    ...
    /// Only apply the reconstruction filter along the Y axis? (misuka:
    /// used for acoustic ETC accumulation, where X indexes discrete bins)
    void set_y_only(bool value) { m_y_only = value; }

    /// Only apply the reconstruction filter along the Y axis?

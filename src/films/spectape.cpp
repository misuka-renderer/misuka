...
public:
    ...
    SpecTape(const Properties &props) : Base(props) {
        if constexpr (!is_spectral_v<Spectrum>)
        Log(Error, "This film can only be used in Mitsuba variants that "
                   "perform a spectral simulation.");

        m_size = ScalarVector2u(
            //Size of one block should correspond to the spectrum size (number of wavelengths per ray).
            //Currently this is always 1, so the stores one time histogram per channel.
            Spectrum::Size,
        ...
        Log(Debug, "Loading sensor response functions ..");
        for (auto &[name, obj] : props.objects(false)) {
            ...
            if (srf != nullptr) {
                ...
                Log(Debug, "Loaded sensor response function \"%s\"", name);
            }
        }
        ...
        // Log(Debug, "Getting component format %s from props..", component_format);

        // The resulting bitmap is always OpenEXR MultiChannel
        // Log(Debug, "Setting file format to OpenEXR and pixel format to MultiChannel ..");
        m_file_format = Bitmap::FileFormat::OpenEXR;
        m_pixel_format = Bitmap::PixelFormat::MultiChannel;

        // Log(Debug, "Setting m_component_format to %s ..", component_format);
        ...
        Log(Debug, "Calling compute_srf_sampling..");
    ...
    void compute_srf_sampling() {
        ...
        Log(Debug, "Computing full range of wavelengths and resolution in the film ..");
        for (auto srf : m_srfs) {
            ...
            Log(Debug, "SRF covers wavelengths [%f, %f] with resolution %f.",
                srf->wavelength_range().x(), srf->wavelength_range().y(), srf->spectral_resolution());
        }
        Log(Debug, "Full range of wavelengths is [%f, %f] with resolution %f.",
            m_range.x(), m_range.y(), resolution);
        ...
        for (auto srf : m_srfs) {
            ...
            // Log(Debug, "duplicating values.x() = %f ..", values.x());
            mis_data += values.x();
        }
    ...
    size_t prepare(const std::vector<std::string>& channels) override {
        Log(Debug, "Preparing spectral tape");
        ...
        /* locked */ {
            Log(Debug, "Sorted channels with added weight channel: %s", sorted);
            m_channels = sorted;
            Log(Debug, "Creating Imageblock with %i channels, crop size %s and crop offset %s..", m_channels.size(), m_crop_size, m_crop_offset);
    ...
    ref<ImageBlock> create_block(const ScalarVector2u &size, bool normalize,
        ...
        return new ImageBlock(default_config ? m_crop_size : size,
                              ...
                              false /* warn_negative */,
                              false /* warn_invalid */,
                              true /* apply reconstruction filter to y_only */);
    }
    ...
    ref<Bitmap> bitmap(bool raw = false) const override {
        ...
        if (raw)
            return source;
        Log(Debug, "Converting bitmap to target format ..");
    ...
    std::string to_string() const override {
        ...
        oss << "SpecTape[" << std::endl
            << "  size = " << m_size << "," << std::endl
            << "  channels = " << base_channels_count() << "," << std::endl
...
MI_IMPLEMENT_CLASS_VARIANT(SpecTape, Film)
MI_EXPORT_PLUGIN(SpecTape, "Spectral Tape")

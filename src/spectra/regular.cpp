...
public:
    RegularSpectrum(const Properties &props) : Texture(props) {
        if (props.has_property("value")) {
            ...
            /* misuka: 'frequency_min'/'frequency_max' (in Hz) may be given as
               an alternative to 'wavelength_min'/'wavelength_max' for acoustic
               rendering. The frequencies are stored in the wavelength slots
               (semantically unclean but intended). */
            ...
            Properties::Spectrum spec(
                ...
                props.get<double>(has_frequency ? "frequency_min" : "wavelength_min"),
                props.get<double>(has_frequency ? "frequency_max" : "wavelength_max")
            );
            init(spec);
        }
    }

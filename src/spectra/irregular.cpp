...
public:
    IrregularSpectrum(const Properties &props) : Texture(props) {
        if (props.has_property("value")) {
            ...
            /* misuka: 'frequencies' (in Hz) may be given as an alternative to
               'wavelengths' for acoustic rendering. The frequencies are stored
               in the wavelength slots (semantically unclean but intended). */
            ...
            Properties::Spectrum spec(
                props.get<std::string_view>(has_frequencies ? "frequencies" : "wavelengths"),
